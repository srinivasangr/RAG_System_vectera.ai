...
|---|---|---|
| [`architecture.drawio`](architecture.drawio) | **High-level diagram** — generated by [`_build_diagram.py`](_build_diagram.py)| Open in [app.diagrams.net](https://app.diagrams.net) → **File → Open** → pick the file. Or use the VS Code **Draw.io Integration** extension. |
| Mermaid block below | **Functional flow** | Just scroll ↓ |
